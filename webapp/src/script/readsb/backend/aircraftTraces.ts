...
namespace READSB {
    ...
    let Frontend: MessagePort = null; // Message port to frontend
        ...
        switch (msg.type) {
            case "HistoryPort":
                ...
                AircraftHistoryWorker.onmessage = (evt: MessageEvent) => {
                    const msg2 = evt.data;
                    if (msg2.type === "Update") {
                        UpdateTrace(msg2.data[0], msg2.data[1], msg2.data[2]);
                    }
                };
                break;
            case "FrontendPort":
                Frontend = msg.data;
                Frontend.onmessage = (evt: MessageEvent) => {
                    const msg2 = evt.data;
                    if (msg2.type === "Get") {
                        GetTrace(msg2.data);
                    }
    ...
    function GetTrace(icao: string) {
        if (AircraftTraceCollection.has(icao)) {
            ...
            Frontend.postMessage({ type: "Trace", data: [icao, AircraftTraceCollection.get(icao).Trace] });
        }
    }
}
